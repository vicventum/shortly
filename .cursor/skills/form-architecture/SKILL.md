...
- **1. Form (Smart Container)**: 
  - Located in `/[module-name]/components/form/Form[Name].tsx|jsx`.
...
- **2. Fields (Dumb Presentation)**:
  - Located in `/[module-name]/components/fields/Fields[Name].tsx|jsx`.
...
1. Create the `Fields[Name]` component in `/[module]/components/fields/`. Design the layout using grids/flexbox and atomic inputs. Expose props to accept field controllers from the parent.
2. Create the `Form[Name]` component in `/[module]/components/form/`. Initialize the form hook with initial values and validators.
...
- Example Form (Smart): `src/modules/dashboard/components/form/FormDashboardUrl.jsx`
- Example Fields (Dumb): `src/modules/settings/components/fields/FieldsProfileSettings.jsx`
