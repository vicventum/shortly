...
- **1. Layout**: Every page MUST be wrapped in a Layout component (from `_core/` or the local module) that defines the shell (navbar, sidebar, max-width wrappers).
...
- **4. Section Components**: Located in `[scope]components/Section[Name].tsx|jsx`. They act as feature containers.
...
| Grouping a feature's UI | Create a **Section** component in `[scope]components/` |
...
3. Create the required `Section` components in `[scope]components/`.
