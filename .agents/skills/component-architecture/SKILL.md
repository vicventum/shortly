...
Located in `src/modules/core/components/`. Uses the **ABCD+Layout System**. The component file and naming MUST reflect the category prefix. Files should use **kebab-case** naming (`a-button.jsx` / `a-button.tsx`).
...
**Core constraints:**
- Components in `core` (A, B, C, D, L) MUST NOT contain business logic, API calls, or global state mutations (e.g., Zustand). 
